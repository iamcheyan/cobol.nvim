...
| **Phase 2** | **结构大纲与快速导航** | Winbar 动态段落/数据面包屑导航、DATA DIVISION 01/88 级高亮与宿主回溯、`PERFORM`/`CALL` 段落直达 (`gd`)、`COPY` 跳转 (`gf`) | **核心完成 (v0.2.0) ✅** |
| **Phase 3** | **数据层级与 PIC 计算器** | `PIC` 字段长度与 Record 总字节数自动求和、`COMP-3` 字节换算、字段对齐校验 | **规划中 (v0.3.0)** |
